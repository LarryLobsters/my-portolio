...
const schema = z.object({
	name: z
		.string()
		.nonempty('I need to know your name')
	...
	company: z.string(),
	message: z.string().nonempty('No message??'),
})
		...
		try {
			...
			setFormResponse({
				...
				message:
					...
					`Sorry ${name}, something went wrong. Please try again. 😢`,
			})
		}
	}
	...
	return (
		...
		>
			<Label required>
				...
				/>
				{errors.name && (
					<span className='dark:text-red-400 block mt-4 font-light text-red-500'>
						{errors.name.message}
					</span>
				)}
			</Label>
			...
			<Label required>
				...
				<Input
					{...register('email')}
					name='email'
					placeholder="Cool person's email"
					className={cn({
			...
			<Label required>
				...
				<Textarea
					{...register('message')}
					name='message'
					placeholder='Hey Larry, I have an idea...'
					className={cn({
						'border-red-500 dark:border-red-400': errors.message?.message,
						'border-green-500 dark:border-green-400':
							isValid && !errors.message?.message,
					})}
				/>
				{errors.message && (
					<span className='dark:text-red-400 block mt-4 font-light text-red-500'>
			...
			{formResponse.status && (
				<p
					className={cn({
						'text-green-500':
							formResponse.status === 'success' && isSubmitSuccessful,
